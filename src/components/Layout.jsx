...
export default function Layout() {
   ...
   // Safety: reset scroll-lock on route changes (fixes stuck scroll on mobile)
   useEffect(() => {
         ...
         <footer className="mt-20 border-t border-ivory/5">
            <div className="mx-auto max-w-6xl px-4 py-10 text-muted text-sm">
               <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                  <div>
                     © {new Date().getFullYear()} Team Adhit — Custom Build
                     Experience.
                  </div>
                  <NavLink
                     to="/admin"
                     className="text-xs tracking-luxe uppercase hover:text-ivory"
                  >
                     Admin
                  </NavLink>
               </div>
