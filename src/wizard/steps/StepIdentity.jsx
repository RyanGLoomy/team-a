...
function Field({ label, value, onChange, placeholder, type = "text" }) {
  return (
    <label className="block">
      <div className="text-xs tracking-luxe uppercase text-muted">{label}</div>
      <input
        type={type}
        value={value}
...
export default function StepIdentity({ value, onChange, onNext, onBack }) {
  const isValid = useMemo(() => {
    return Boolean(value.fullName?.trim() && value.phone?.trim() && value.contactPerson?.trim());
  }, [value.fullName, value.phone, value.contactPerson]);

  return (
    <div>
      <div className="text-xs tracking-luxe uppercase text-muted">Step 04</div>
      <h2 className="mt-2 text-2xl md:text-3xl">Identitas calon konsumen</h2>
      <p className="mt-3 text-muted leading-relaxed">
        Data ini digunakan sebagai bahan komunikasi Team Adhit dengan calon konsumen.
      </p>
      ...
      <div className="mt-4">
        <div className="text-xs tracking-luxe uppercase text-muted">Alamat</div>
        <textarea
          value={value.address}
      ...
      <div className="mt-4">
        <div className="text-xs tracking-luxe uppercase text-muted">Catatan tambahan</div>
        <textarea
          value={value.notes}
      ...
      <div className="mt-7 flex items-center justify-between gap-3">
        ...
        <button
          ...
          className={[
            ...
            isValid ? "bg-rolex hover:brightness-110" : "bg-ivory/10 text-muted cursor-not-allowed",
          ].join(" ")}
        >
          Next
        </button>
