...
export default function AdminLeadDetail() {
   ...
   if (loading) {
      return <div className="py-10 md:py-14 text-muted">Loading…</div>;
   }
      ...
      return (
         <div className="py-10 md:py-14">
            ...
            <div className="mt-4">
               <Link
                  to="/admin"
                  className="underline text-ivory/80 hover:text-ivory"
               >
                  ← Back
               </Link>
            </div>
   ...
   if (!row) {
      return <div className="py-10 md:py-14 text-muted">Not found.</div>;
   }
      ...
      <div className="py-10 md:py-14">
         <div className="flex items-start justify-between gap-4">
            <div>
               ...
               <h1 className="mt-2 text-2xl md:text-3xl text-ivory">
                  {row.full_name}
               </h1>
               <div className="mt-1 text-sm text-muted tabular-nums">
                  {new Date(row.created_at).toLocaleString("id-ID")} · ID:{" "}
                  {row.id}
               </div>
            </div>
            ...
            <div className="lg:col-span-7 rounded-2xl bg-graphite border border-ivory/10 p-5 shadow-soft">
               ...
               <div className="mt-4 grid md:grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                     ...
                     <div className="text-ivory uppercase">
                        {row.payment_method}
                     </div>
                  </div>
                  <div>
                  ...
                  <div>
                     ...
                     <div className="text-ivory tabular-nums">
                        {row.dp_amount ? formatIDR(row.dp_amount) : "-"}
                        {row.dp_percent != null ? ` (${row.dp_percent}%)` : ""}
                     </div>
                  </div>
                  <div>
                     <div className="text-muted">Tenor</div>
                     <div className="text-ivory tabular-nums">
                        {row.tenor_months ? `${row.tenor_months} bulan` : "-"}
                     </div>
                  </div>
                  <div>
                     ...
                     <div className="text-ivory tabular-nums">
                        {row.monthly_installment
                           ? formatIDR(row.monthly_installment)
                           : "-"}
                     </div>
                  </div>
                  <div>
